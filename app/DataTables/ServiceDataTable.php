        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function($record){
                ...
                if (Gate::check('service_edit')) {
                    $actionHtml .= '<a href="'.route('admin.services.edit',$record->id).'" class="btn btn-outline-success btn-sm" title="Edit"><i class="ri-edit-2-line"></i></a>';
                }
                if (Gate::check('service_delete')) {
				    $actionHtml .= '<button type="button" class="btn btn-outline-danger btn-sm deleteServiceBtn" data-href="'.route('admin.services.destroy', $record->id).'" title="Delete"><i class="ri-delete-bin-line"></i></button>';
                }
                return $actionHtml;
            })
            ->setRowId('id')
