...
class StoreRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        if($this->step_no == 1){
            $rules['company_name']          = ['required', 'regex:/^[a-zA-Z\s]+$/','string',  new NoMultipleSpacesRule, Rule::unique('companies', 'company_name')->whereNull('deleted_at')];
            ...
            $rules['email']                 = ['required', 'email', 'regex:/^(?!.*[\/]).+@(?!.*[\/]).+\.(?!.*[\/]).+$/i', Rule::unique('users', 'email')->whereNull('deleted_at')];
            
            $rules['phone']                 = ['nullable', 'numeric', 'regex:/^[0-9]{7,15}$/', Rule::unique('users', 'phone')->whereNull('deleted_at')];
